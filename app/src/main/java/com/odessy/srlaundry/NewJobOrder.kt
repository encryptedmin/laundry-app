...
class NewJobOrder : AppCompatActivity() {
    ...
    private suspend fun createJobOrder() {
        ...
        // If promotion is active and customer is eligible
        ...
        if (promotion != null && promotion.isPromoActive && selectedCustomer != null) {
            if (selectedCustomer!!.promo >= promotion.serviceFrequency) {
                // Apply promo by deducting the price of one load
                finalTotalPrice -= (laundryPrice.regular) // Assuming the free load applies to regular service
                isPromoApplied = true
                selectedCustomer!!.promo = 0 // Reset promo count after applying
            } else {
                selectedCustomer!!.promo += 1 // Increment promo count
            }
        }
